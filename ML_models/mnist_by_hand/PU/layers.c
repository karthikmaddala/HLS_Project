...
void k2c_maxpool2d(k2c_tensor* output, const k2c_tensor* input, const size_t * pool_size,
    ...
    for (size_t i = 0; i < channels; ++i) {
        ...
        for (size_t j = 0; j < output->shape[1]; ++j) {
            ...
            for (size_t l = 0; l < output->shape[0]; ++l) {
                #pragma HLS loop_tripcount min=13 max=13

                float max_val = -FLT_MAX; // Initialize to the minimum possible float value
                size_t base_input_index = (l * stride[0] * input->shape[1] + j * stride[1]) * channels + i;
                size_t base_output_index = (l * output->shape[1] + j) * channels + i;
...
#pragma HLS loop_tripcount min=2 max=2
                        #pragma HLS UNROLL
                        size_t input_index = base_input_index + (n * input->shape[1] + p) * channels;
