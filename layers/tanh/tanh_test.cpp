...
// Function to compute tanh manually for verification
void compute_tanh(const float *input, float *output, size_t size) {
    for (size_t i = 0; i < size; ++i) {
...
int main() {
    ...
    // Compute the expected output using the manual tanh function
    compute_tanh(input_array, expected_output_array, array_size);

    // Print input array (optional)
    printf("Input array:\n");
    print_array(input_array, array_size);

    // Call the tanh function
    k2c_tanh_func(output_array, array_size);
