...
k2c_tensor conv2d_31_output = {0};
k2c_tensor conv2d_31_kernel = {0};
k2c_tensor conv2d_31_bias = {0};
k2c_tensor conv2d_31_padded_input = {0};
k2c_tensor max_pooling2d_18_output = {0};
k2c_tensor conv2d_32_output = {0};
k2c_tensor conv2d_32_padded_input = {0};
k2c_tensor conv2d_32_kernel = {0};
k2c_tensor conv2d_32_bias = {0};
k2c_tensor max_pooling2d_19_output = {0};
k2c_tensor conv2d_33_output = {0};
k2c_tensor conv2d_33_padded_input = {0};
k2c_tensor conv2d_33_kernel = {0};
k2c_tensor conv2d_33_bias = {0};
k2c_tensor conv2d_34_output = {0};
k2c_tensor conv2d_34_padded_input = {0};
k2c_tensor conv2d_34_kernel = {0};
k2c_tensor conv2d_34_bias = {0};
k2c_tensor flatten_8_output = {0};
...
k2c_tensor dense_31_output = {0};
k2c_tensor dense_31_kernel = {0};
k2c_tensor dense_31_bias = {0};
k2c_tensor dense_32_kernel = {0};
k2c_tensor dense_32_bias = {0};
...
void k2c_pad2d(k2c_tensor* output, const k2c_tensor* input, const float fill, const size_t * pad) {
    ...
    if (fabs(fill) < 1e-6) {
        for (size_t i1 = 0; i1 < output->numel; ++i1) {
            #pragma HLS loop_tripcount min=10404 max=10404
            output->array[i1] = 0.0f;
        }
    } else {
        for (size_t i2 = 0; i2 < output->numel; ++i2) {
            #pragma HLS loop_tripcount min=10404 max=10404
            output->array[i2] = fill;
        }
    }
    ...
    for (size_t i3 = 0; i3 < in_height; ++i3) {
        #pragma HLS loop_tripcount min=102 max=102
        size_t inner_offset = offset + i3 * step;
        for (size_t j1 = 0; j1 < num; ++j1) {
            #pragma HLS loop_tripcount min=102 max=102
            output->array[inner_offset + j1] = input->array[i3 * num + j1];
        }
    }
}
...
void k2c_maxpool2d(k2c_tensor* output, const k2c_tensor* input, const size_t * pool_size, const size_t * stride) {
    ...
    const size_t output_width = output->shape[1] * channels;
    const size_t input_width = input->shape[1] * channels;
    const size_t pool_width = pool_size[1] * channels;
    const size_t pool_height = pool_size[0] * channels * input->shape[1];

    for (size_t i4 = 0; i4 < channels; ++i4) {
        #pragma HLS loop_tripcount min=6 max=6
        for (size_t j2 = 0, k = 0; j2 < output_width; j2 += channels, k += channels * stride[1]) {
            ...
            for (size_t l = 0, m = 0; l < output->numel; l += channels * output->shape[1], m += channels * input->shape[1] * stride[0]) {
                #pragma HLS loop_tripcount min=25 max=25
                float max_value = input->array[m + k + i4];
                for (size_t n = 0; n < pool_width; n += channels) {
                    #pragma HLS loop_tripcount min=2 max=2
                    for (size_t p = 0; p < pool_height; p += channels * input->shape[1]) {
                        #pragma HLS loop_tripcount min=2 max=2
                        float current_value = input->array[m + k + i4 + n + p];
                        if (max_value < current_value) {
                            max_value = current_value;
                        }
                    }
                }
                output->array[l + j2 + i4] = max_value;
            }
        }
    }
}
...
void k2c_flatten(k2c_tensor *output, const k2c_tensor* input) {
    for (size_t i5 = 0; i5 < input->numel; ++i5) {
        #pragma HLS loop_tripcount min=15000 max=15000
        output->array[i5] = input->array[i5];
    }
    for (size_t i6 = 0; i6 < input->ndim; ++i6) {
...
void k2c_matmul(float * C, const float * A, const float * B, const size_t outrows, const size_t outcols, const size_t innerdim) {
    for (size_t i7 = 0; i7 < outrows * outcols; ++i7) {
        #pragma HLS loop_tripcount min=15000 max=15000
        C[i7] = 0.0f;
    }

    for (size_t i8 = 0; i8 < outrows; ++i8) {
        #pragma HLS loop_tripcount min=50 max=50
        const size_t outrowidx = i8 * outcols;
        const size_t inneridx = i8 * innerdim;
...
void k2c_affine_matmul(float * C, const float * A, const float * B, const float * d, const size_t outrows, const size_t outcols, const size_t innerdim) {
    for (size_t i10 = 0; i10 < outrows * outcols; ++i10) {
        #pragma HLS loop_tripcount min=70 max=70
        C[i10] = 0.0f;
    }

    for (size_t i11 = 0; i11 < outrows; ++i11) {
        #pragma HLS loop_tripcount min=10 max=10
        const size_t outrowidx = i11 * outcols;
        const size_t inneridx = i11 * innerdim;
...
void k2c_dot(k2c_tensor* C, const k2c_tensor* A, const k2c_tensor* B, const size_t * axesA, const size_t * axesB, const size_t naxes, const int normalize, float * fwork) {
    ...
    count = 0;
    for (size_t i14 = 0; i14 < ndimA; ++i14) {
        #pragma HLS loop_tripcount min=3 max=3
        isin = 0;
    ...
    count = 0;
    for (size_t i15 = 0; i15 < ndimB; ++i15) {
        #pragma HLS loop_tripcount min=3 max=3
        isin = 0;
    ...
    for (size_t i16 = 0; i16 < naxes; ++i16) {
        #pragma HLS loop_tripcount min=1 max=1
        prod_axesA *= A->shape[axesA[i16]];
    }
    for (size_t i17 = 0; i17 < naxes; ++i17) {
        #pragma HLS loop_tripcount min=1 max=1
        prod_axesB *= B->shape[axesB[i17]];
    }

    free_axesA = A->numel / prod_axesA;
    free_axesB = B->numel / prod_axesB;

    for (size_t i18 = 0; i18 < ndimA - naxes; ++i18) {
        #pragma HLS loop_tripcount min=3 max=3
        permA[i18] = freeA[i18];
    }
    for (size_t i19 = ndimA - naxes, j8 = 0; i19 < ndimA; ++i19, ++j8) {
        #pragma HLS loop_tripcount min=3 max=3
        permA[i19] = axesA[j8];
    }
    for (size_t i20 = 0; i20 < naxes; ++i20) {
        #pragma HLS loop_tripcount min=1 max=1
        permB[i20] = axesB[i20];
    }
    for (size_t i21 = naxes, j9 = 0; i21 < ndimB; ++i21, ++j9) {
        #pragma HLS loop_tripcount min=3 max=3
        permB[i21] = freeB[j9];
    }

    for (size_t i22 = 0; i22 < ndimA; ++i22) {
        #pragma HLS loop_tripcount min=3 max=3
        newshpA[i22] = A->shape[permA[i22]];
    }
    for (size_t i23 = 0; i23 < ndimB; ++i23) {
        #pragma HLS loop_tripcount min=3 max=3
        newshpB[i23] = B->shape[permB[i23]];
    }

    for (size_t i24 = 0; i24 < A->numel; ++i24) {
        #pragma HLS loop_tripcount min=15000 max=15000
        k2c_idx2sub(i24, Asub, A->shape, ndimA);
        for (size_t j10 = 0; j10 < ndimA; ++j10) {
            #pragma HLS loop_tripcount min=3 max=3
            Bsub[j10] = Asub[permA[j10]];
        }
        size_t bidx = k2c_sub2idx(Bsub, newshpA, ndimA);
        reshapeA[bidx] = A->array[i24];
    }

    for (size_t i25 = 0; i25 < B->numel; ++i25) {
        #pragma HLS loop_tripcount min=15000 max=15000
        k2c_idx2sub(i25, Bsub, B->shape, ndimB);
        for (size_t j11 = 0; j11 < ndimB; ++j11) {
            #pragma HLS loop_tripcount min=3 max=3
            Asub[j11] = Bsub[permB[j11]];
        }
        size_t bidx = k2c_sub2idx(Asub, newshpB, ndimB);
        reshapeB[bidx] = B->array[i25];
    }

    if (normalize) {
        float sum;
        float inorm;
        for (size_t i26 = 0; i26 < free_axesA; ++i26) {
            #pragma HLS loop_tripcount min=3 max=3
            sum = 0;
            for (size_t j12 = 0; j12 < prod_axesA; ++j12) {
                #pragma HLS loop_tripcount min=1 max=1
                sum += reshapeA[i26 * prod_axesA + j12] * reshapeA[i26 * prod_axesA + j12];
            }
            inorm = 1.0f / sqrtf(sum);
            for (size_t j13 = 0; j13 < prod_axesA; ++j13) {
                #pragma HLS loop_tripcount min=1 max=1
                reshapeA[i26 * prod_axesA + j13] *= inorm;
            }
        }
        for (size_t i27 = 0; i27 < free_axesB; ++i27) {
            #pragma HLS loop_tripcount min=3 max=3
            sum = 0;
            for (size_t j14 = 0; j14 < prod_axesB; ++j14) {
                #pragma HLS loop_tripcount min=1 max=1
                sum += reshapeB[i27 + free_axesB * j14] * reshapeB[i27 + free_axesB * j14];
            }
            inorm = 1.0f / sqrtf(sum);
            for (size_t j15 = 0; j15 < prod_axesB; ++j15) {
                #pragma HLS loop_tripcount min=1 max=1
                reshapeB[i27 + free_axesB * j15] *= inorm;
            }
        }
    }
...
void k2c_idx2sub(const size_t idx, size_t * sub, const size_t * shape, const size_t ndim) {
    size_t remaining_idx = idx;
    for (int i28 = ndim - 1; i28 >= 0; --i28) {
        #pragma HLS loop_tripcount min=3 max=3
        sub[i28] = remaining_idx % shape[i28];
        remaining_idx /= shape[i28];
    }
}
...
void k2c_conv2d(k2c_tensor* output, const k2c_tensor* input, const k2c_tensor* kernel, const k2c_tensor* bias, const size_t * stride, const size_t * dilation, int activation) {
    for (size_t i30 = 0; i30 < output->numel; ++i30) {
        #pragma HLS loop_tripcount min=15000 max=15000
        output->array[i30] = 0.0f;
    }
...
void k2c_pad2d(k2c_tensor* output, const k2c_tensor* input, const float fill, const size_t * pad) {
    ...
    // set output array to fill value
    if (fabs(fill) < 1e-6) {
        // fill is ~zero, use memset equivalent
        for (size_t i1 = 0; i1 < output->numel; ++i1) {
            #pragma HLS loop_tripcount min=10404 max=10404
    ...
    for (size_t i3 = 0; i3 < in_height; ++i3) {
        ...
        for (size_t j1 = 0; j1 < num; ++j1) {
            ...
            #pragma HLS UNROLL factor=2
            #pragma HLS PIPELINE
            output->array[offset + j1] = input->array[i3 * num + j1];
        }
        offset += step;
    }
}
void k2c_maxpool2d(k2c_tensor* output, const k2c_tensor* input, const size_t * pool_size, const size_t * stride) {
    ...
    // i3, k, m input indices
    for (size_t i4 = 0; i4 < channels; ++i4) {
        #pragma HLS loop_tripcount min=6 max=6
        #pragma HLS PIPELINE
        for (size_t j2 = 0, k = 0; j2 < output->shape[1] * channels; j2 += channels, k += channels * stride[1]) {
            #pragma HLS loop_tripcount min=25 max=25
            #pragma HLS PIPELINE
            for (size_t l = 0, m = 0; l < output->numel; l += channels * output->shape[1], m += channels * input->shape[1] * stride[0]) {
                #pragma HLS loop_tripcount min=25 max=25
                #pragma HLS PIPELINE
                output->array[l + j2 + i4] = input->array[m + k + i4];
                for (size_t n = 0; n < pool_size[1] * channels; n += channels) {
                    ...
                    for (size_t p = 0; p < pool_size[0] * channels * input->shape[1]; p += channels * input->shape[1]) {
                        ...
                        #pragma HLS UNROLL factor=2
                        #pragma HLS PIPELINE
                        if (output->array[l + j2 + i4] < input->array[m + k + i4 + n + p]) {
                            output->array[l + j2 + i4] = input->array[m + k + i4 + n + p];
                        }
                    }
                }
            }
...
size_t k2c_sub2idx(const size_t * sub, const size_t * shape, const size_t ndim) {
    size_t idx = 0;
    size_t temp = 0;
    for (size_t i29 = 0; i29 < ndim; ++i29) {
        #pragma HLS loop_tripcount min=3 max=3
        #pragma HLS PIPELINE
        #pragma HLS UNROLL factor=2
        temp = sub[i29];
        for (size_t j16 = ndim - 1; j16 > i29; --j16) {
            #pragma HLS loop_tripcount min=3 max=3
            #pragma HLS PIPELINE
            #pragma HLS UNROLL factor=2
            temp *= shape[j16];
...
void k2c_maxpool2d(k2c_tensor* output, const k2c_tensor* input, const size_t * pool_size,
    ...
    // i3, k, m input indices
    for (size_t i4 = 0; i4 < channels; ++i4) {
        for (size_t j2 = 0, k = 0; j2 < output->shape[1] * channels;
                j2 += channels, k += channels * stride[1]) {
...
*/
void finalbraintumor(k2c_tensor* input_9_input, k2c_tensor* dense_32_output) { 
...
// Copy the contents of conv2d_31_output_array into conv2d_31_output.array
for (size_t i500 = 0; i500 < 15000; ++i500) {
    conv2d_31_output.array[i500] = conv2d_31_output_array[i500];
}

// Set the other members of conv2d_31_output
...
// Copy the contents of conv2d_31_padded_input_array into conv2d_31_padded_input.array
for (size_t i503 = 0; i503 < 10404; ++i503) {
    conv2d_31_padded_input.array[i503] = conv2d_31_padded_input_array[i503];
}

// Set the other members of conv2d_31_padded_input
...
// Copy the contents of conv2d_31_kernel_array into conv2d_31_kernel.array
for (size_t i501 = 0; i501 < 54; ++i501) {
    conv2d_31_kernel.array[i501] = conv2d_31_kernel_array[i501];
}

// Set the other members of conv2d_31_kernel
...
// Copy the contents of conv2d_31_bias_array into conv2d_31_bias.array
for (size_t i502 = 0; i502 < 6; ++i502) {
    conv2d_31_bias.array[i502] = conv2d_31_bias_array[i502];
}

// Set the other members of conv2d_31_bias
...
// Copy the contents of max_pooling2d_18_output_array into max_pooling2d_18_output.array
for (size_t i504 = 0; i504 < 3750; ++i504) {
    max_pooling2d_18_output.array[i504] = max_pooling2d_18_output_array[i504];
}

// Set the other members of max_pooling2d_18_output
...
// Copy the contents of conv2d_32_output_array into conv2d_32_output.array
for (size_t i505 = 0; i505 < 1014; ++i505) {
    conv2d_32_output.array[i505] = conv2d_32_output_array[i505];
}

// Set the other members of conv2d_32_output
...
// Copy the contents of conv2d_32_padded_input_array into conv2d_32_padded_input.array
for (size_t i506 = 0; i506 < 4374; ++i506) {
    conv2d_32_padded_input.array[i506] = conv2d_32_padded_input_array[i506];
}

// Set the other members of conv2d_32_padded_input
...
// Copy the contents of conv2d_32_kernel_array into conv2d_32_kernel.array
for (size_t i507 = 0; i507 < 324; ++i507) {
    conv2d_32_kernel.array[i507] = conv2d_32_kernel_array[i507];
}

// Set the other members of conv2d_32_kernel
...
// Copy the contents of conv2d_32_bias_array into conv2d_32_bias.array
for (size_t i508 = 0; i508 < 6; ++i508) {
    conv2d_32_bias.array[i508] = conv2d_32_bias_array[i508];
}

// Set the other members of conv2d_32_bias
...
// Copy the contents of max_pooling2d_19_output_array into max_pooling2d_19_output.array
for (size_t i509 = 0; i509 < 216; ++i509) {
    max_pooling2d_19_output.array[i509] = max_pooling2d_19_output_array[i509];
}

// Set the other members of max_pooling2d_19_output
...
// Copy the contents of conv2d_33_output_array into conv2d_33_output.array
for (size_t i510 = 0; i510 < 54; ++i510) {
    conv2d_33_output.array[i510] = conv2d_33_output_array[i510];
}

// Set the other members of conv2d_33_output
...
// Copy the contents of conv2d_33_padded_input_array into conv2d_33_padded_input.array
for (size_t i511 = 0; i511 < 384; ++i511) {
    conv2d_33_padded_input.array[i511] = conv2d_33_padded_input_array[i511];
}

// Set the other members of conv2d_33_padded_input
...
// Copy the contents of conv2d_33_kernel_array into conv2d_33_kernel.array
for (size_t i512 = 0; i512 < 324; ++i512) {
    conv2d_33_kernel.array[i512] = conv2d_33_kernel_array[i512];
}

// Set the other members of conv2d_33_kernel
...
// Copy the contents of conv2d_33_bias_array into conv2d_33_bias.array
for (size_t i513 = 0; i513 < 6; ++i513) {
    conv2d_33_bias.array[i513] = conv2d_33_bias_array[i513];
}

// Set the other members of conv2d_33_bias
...
// Copy the contents of conv2d_34_output_array into conv2d_34_output.array
for (size_t i514 = 0; i514 < 28; ++i514) {
    conv2d_34_output.array[i514] = conv2d_34_output_array[i514];
}

// Set the other members of conv2d_34_output
...
// Copy the contents of conv2d_34_padded_input_array into conv2d_34_padded_input.array
for (size_t i515 = 0; i515 < 150; ++i515) {
    conv2d_34_padded_input.array[i515] = conv2d_34_padded_input_array[i515];
}

// Set the other members of conv2d_34_padded_input
...
// Copy the contents of conv2d_34_kernel_array into conv2d_34_kernel.array
for (size_t i516 = 0; i516 < 378; ++i516) {
    conv2d_34_kernel.array[i516] = conv2d_34_kernel_array[i516];
}

// Set the other members of conv2d_34_kernel
...
// Copy the contents of conv2d_34_bias_array into conv2d_34_bias.array
for (size_t i517 = 0; i517 < 7; ++i517) {
    conv2d_34_bias.array[i517] = conv2d_34_bias_array[i517];
}

// Set the other members of conv2d_34_bias
...
// Copy the contents of flatten_8_output_array into flatten_8_output.array
for (size_t i518 = 0; i518 < 28; ++i518) {
    flatten_8_output.array[i518] = flatten_8_output_array[i518];
}

// Set the other members of flatten_8_output
...
for (size_t i519 = 0; i519 < 84; ++i519) {
    dense_29_output.array[i519] = dense_29_output_array[i519];
}
...
for (size_t i520 = 0; i520 < 2352; ++i520) {
    dense_29_kernel.array[i520] = dense_29_kernel_array[i520];
}
...
for (size_t i521 = 0; i521 < 84; ++i521) {
    dense_29_bias.array[i521] = dense_29_bias_array[i521];
}
...
for (size_t i522 = 0; i522 < 70; ++i522) {
    dense_30_output.array[i522] = dense_30_output_array[i522];
}
...
for (size_t i523 = 0; i523 < 5880; ++i523) {
    dense_30_kernel.array[i523] = dense_30_kernel_array[i523];
}
...
for (size_t i524 = 0; i524 < 70; ++i524) {
    dense_30_bias.array[i524] = dense_30_bias_array[i524];
}

// Set the other members of dense_30_bias
dense_30_bias.ndim = 1;
dense_30_bias.numel = 70;
dense_30_bias.shape[0] = 70;
...
// Copy the contents of dense_31_output_array into dense_31_output.array
for (size_t i525 = 0; i525 < 10; ++i525) {
    dense_31_output.array[i525] = dense_31_output_array[i525];
}

// Set the other members of dense_31_output
...
for (size_t i526 = 0; i526 < 700; ++i526) {
    dense_31_kernel.array[i526] = dense_31_kernel_array[i526];
}
...
for (size_t i527 = 0; i527 < 10; ++i527) {
    dense_31_bias.array[i527] = dense_31_bias_array[i527];
}
...
// Copy the contents of dense_32_kernel_array into dense_32_kernel.array
for (size_t i528 = 0; i528 < 10; ++i528) {
    dense_32_kernel.array[i528] = dense_32_kernel_array[i528];
}

// Set the other members of dense_32_kernel
...
// Copy the contents of dense_32_bias_array into dense_32_bias.array
for (size_t i529 = 0; i529 < 1; ++i529) {
    dense_32_bias.array[i529] = dense_32_bias_array[i529];
}

// Set the other members of dense_32_bias
...
k2c_conv2d(&conv2d_31_output,&conv2d_31_padded_input,&conv2d_31_kernel, 
	&conv2d_31_bias,conv2d_31_stride,conv2d_31_dilation,RELU);
...
k2c_conv2d(&conv2d_32_output,&conv2d_32_padded_input,&conv2d_32_kernel, 
	&conv2d_32_bias,conv2d_32_stride,conv2d_32_dilation,RELU);
...
k2c_conv2d(&conv2d_33_output,&conv2d_33_padded_input,&conv2d_33_kernel, 
	&conv2d_33_bias,conv2d_33_stride,conv2d_33_dilation,RELU);
...
k2c_conv2d(&conv2d_34_output,&conv2d_34_padded_input,&conv2d_34_kernel, 
	&conv2d_34_bias,conv2d_34_stride,conv2d_34_dilation,RELU);
...
k2c_dense(&dense_31_output,&dense_30_output,&dense_31_kernel, 
	&dense_31_bias,RELU,dense_31_fwork);
k2c_dense(dense_32_output,&dense_31_output,&dense_32_kernel, 
	&dense_32_bias,SIGMOID,dense_32_fwork);

 } 

void finalbraintumor_initialize() { 

} 

void finalbraintumor_terminate() { 
