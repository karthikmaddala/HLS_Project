    ...
    for (size_t i4 = 0; i4 < channels; ++i4) {
        ...
        for (size_t out_y = 0; out_y < out_height; ++out_y) {
            ...
            for (size_t out_x = 0; out_x < out_width; ++out_x) {
                ...
                size_t out_idx = (out_y * out_width + out_x) * channels + i4;

                // Initialize max value with the first element in the pool
                float max_val = -FLT_MAX;

                for (size_t p_y = 0; p_y < pool_size[0]; ++p_y) {
                    #pragma HLS loop_tripcount min=2 max=2
                    for (size_t p_x = 0; p_x < pool_size[1]; ++p_x) {
                        #pragma HLS loop_tripcount min=2 max=2
                        size_t in_y = out_y * stride[0] + p_y;
                        size_t in_x = out_x * stride[1] + p_x;
                        size_t in_idx = (in_y * in_width + in_x) * channels + i4;

                        if (input->array[in_idx] > max_val) {
                            max_val = input->array[in_idx];
                        }
                    }
                }

                output->array[out_idx] = max_val;
            }
        }
    }
}
...
#pragma HLS pipeline II=1
            ...
            for (size_t k = 0; k < innerdim; ++k) {
                #pragma HLS loop_tripcount min=3 max=3
#pragma HLS unroll
                sum += A[inneridx + k] * B[k * outcols + j3];
            }
