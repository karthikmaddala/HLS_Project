    ...
    for (size_t i3 = 0; i3 < channels; ++i3) {
        ...
        for (size_t j2 = 0, k = 0; j2 < output->shape[1] * channels; j2 += channels, k += channels * stride[1]) {
            ...
            for (size_t l = 0, m = 0; l < output->numel; l += channels * output->shape[1], m += channels * input->shape[1] * stride[0]) {
                ...
                output->array[l + j2 + i3] = input->array[m + k + i3];
                for (size_t n = 0; n < pool_size[1] * channels; n += channels) {
                    ...
                    for (size_t p = 0; p < pool_size[0] * channels * input->shape[1]; p += channels * input->shape[1]) {
                        #pragma HLS LOOP_TRIPCOUNT min=1 max=6
                        if (output->array[l + j2 + i3] < input->array[m + k + i3 + n + p]) {
                            output->array[l + j2 + i3] = input->array[m + k + i3 + n + p];
                        }
                    }
                }
            }
...
void k2c_flatten(k2c_tensor *output, const k2c_tensor* input) {
    ...
    for (size_t i5 = 0; i5 < input->ndim; ++i5) {
...
size_t k2c_sub2idx(const size_t * sub, const size_t * shape, const size_t ndim) {
    size_t idx = 0;
    size_t temp = 0;
    for (size_t i31 = 0; i31 < ndim; ++i31) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=3
        temp = sub[i31];
        for (size_t j16 = ndim - 1; j16 > i31; --j16) {
            #pragma HLS LOOP_TRIPCOUNT min=1 max=3
            temp *= shape[j16];
        }
        idx += temp;
...
void k2c_pad2d(k2c_tensor* output, const k2c_tensor* input, const float fill, const size_t * pad) {
    ...
    // Calculate constants outside the loop to avoid recomputation
    const size_t num = in_channels * in_width;
    const size_t step = num + in_channels * (pad_left + pad_right);
    size_t offset = in_channels * (pad_left + pad_right + in_width) * pad_top +
                    in_channels * pad_left;
    ...
    // Copy the input array to the output array in the appropriate position
    for (size_t i2 = 0; i2 < in_height; ++i2) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=102
        for (size_t j1 = 0; j1 < num; ++j1) {
            #pragma HLS LOOP_TRIPCOUNT min=1 max=102
...
void k2c_maxpool2d(k2c_tensor* output, const k2c_tensor* input, const size_t * pool_size, const size_t * stride) {
    const size_t channels = input->shape[2];
    const size_t output_height = output->shape[0];
    const size_t output_width = output->shape[1];
    const size_t input_width = input->shape[1];

    for (size_t i3 = 0; i3 < channels; ++i3) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=6
        for (size_t j2 = 0; j2 < output_width; ++j2) {
            #pragma HLS LOOP_TRIPCOUNT min=1 max=150
            for (size_t l = 0; l < output_height; ++l) {
                #pragma HLS LOOP_TRIPCOUNT min=1 max=3750

                size_t out_index = (l * output_width + j2) * channels + i3;
                size_t in_index_base = (l * stride[0] * input_width + j2 * stride[1]) * channels + i3;

                // Initialize the output array element with the first element in the pool
                output->array[out_index] = input->array[in_index_base];
                ...
                for (size_t n = 0; n < pool_size[0]; ++n) {
                    ...
                    for (size_t p = 0; p < pool_size[1]; ++p) {
                        ...
                        size_t in_index = in_index_base + (n * input_width + p) * channels;
...
void k2c_flatten(k2c_tensor *output, const k2c_tensor* input) {
    // Merged loops for copying elements and setting shape
    for (size_t i4 = 0; i4 < input->numel; ++i4) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=16
        output->array[i4] = input->array[i4];
    }

    // Set the output shape and related parameters
    output->shape[0] = input->numel;
...
void k2c_matmul(float * C, const float * A, const float * B, const size_t outrows, const size_t outcols, const size_t innerdim) {
    ...
    // Optimized loop structure for matrix multiplication
    for (size_t i7 = 0; i7 < outrows; ++i7) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=84
        const size_t outrowidx = i7 * outcols;
        for (size_t j3 = 0; j3 < outcols; ++j3) {
            ...
            for (size_t k = 0; k < innerdim; ++k) {
                #pragma HLS LOOP_TRIPCOUNT min=1 max=84
                C[outrowidx + j3] += A[i7 * innerdim + k] * B[k * outcols + j3];
            }
        }
    }
}
void k2c_bias_add(k2c_tensor* A, const k2c_tensor* b) {
    // Merged and simplified loop for bias addition
    for (size_t i8 = 0; i8 < A->numel; ++i8) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=16
        A->array[i8] += b->array[i8 % b->numel];
    }
}
void k2c_affine_matmul(float * C, const float * A, const float * B, const float * d, const size_t outrows, const size_t outcols, const size_t innerdim) {
    // Initialization of the output matrix
    ...
    // Optimized loop structure for matrix multiplication with bias addition
    for (size_t i10 = 0; i10 < outrows; ++i10) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=84
        const size_t outrowidx = i10 * outcols;
        for (size_t k = 0; k < innerdim; ++k) {
            ...
            for (size_t j5 = 0; j5 < outcols; ++j5) {
                #pragma HLS LOOP_TRIPCOUNT min=1 max=10
                C[outrowidx + j5] += A[i10 * innerdim + k] * B[k * outcols + j5];
            }
        }
        // Bias addition
        for (size_t j5 = 0; j5 < outcols; ++j5) {
...
void k2c_relu_func(float * x, const size_t size) {
    // Optimized loop for ReLU function
    for (size_t i11 = 0; i11 < size; ++i11) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=15000
        x[i11] = (x[i11] > 0.0f) ? x[i11] : 0.0f;
    }
}
void k2c_softmax_func(float * x, const size_t size) {
    float xmax = x[0];
    ...
    // Combined finding xmax and calculating exponentials in one loop
    for (size_t i12 = 0; i12 < size; ++i12) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=10
        if (x[i12] > xmax) {
            xmax = x[i12];
    ...
    // Compute the exponentials and accumulate the sum in one loop
    for (size_t i13 = 0; i13 < size; ++i13) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=10
        x[i13] = expf(x[i13] - xmax);
        sum += x[i13];
    ...
    // Normalize the result in a final loop
    sum = 1.0f / sum;
    for (size_t i14 = 0; i14 < size; ++i14) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=10
        x[i14] *= sum;
...
void k2c_dot(k2c_tensor* C, const k2c_tensor* A, const k2c_tensor* B, const size_t * axesA, const size_t * axesB, const size_t naxes, const int normalize, float * fwork) {
    ...
    // Calculate products of axes
    for (size_t i18 = 0; i18 < naxes; ++i18) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=1
        prod_axesA *= A->shape[axesA[i18]];
        prod_axesB *= B->shape[axesB[i18]];
    }
    ...
    if (normalize) {
        ...
        for (size_t i29 = 0; i29 < free_axesB; ++i29) {
            ...
            for (size_t j14 = 0; j14 < prod_axesB; ++j14) {
                #pragma HLS LOOP_TRIPCOUNT min=1 max=3
                sum += reshapeB[i29 * prod_axesB + j14] * reshapeB[i29 * prod_axesB + j14];
            }
            float inorm = 1.0f / sqrtf(sum);
            for (size_t j15 = 0; j15 < prod_axesB; ++j15) {
                #pragma HLS LOOP_TRIPCOUNT min=1 max=3
                reshapeB[i29 * prod_axesB + j15] *= inorm;
            }
        }
    }
...
void k2c_conv2d(k2c_tensor* output, const k2c_tensor* input, const k2c_tensor* kernel, const k2c_tensor* bias, const size_t * stride, const size_t * dilation, int activation) {
    ...
    const size_t out_channels = output->shape[2];
    const size_t in_channels = input->shape[2];
    const size_t kernel_rows = kernel->shape[0];
    const size_t kernel_cols = kernel->shape[1];
    ...
    for (size_t x0 = 0; x0 < out_rows; ++x0) {
        ...
        for (size_t x1 = 0; x1 < out_cols; ++x1) {
            ...
            for (size_t k = 0; k < out_channels; ++k) {
                #pragma HLS LOOP_TRIPCOUNT min=1 max=6
                float sum = 0.0f;
                for (size_t z0 = 0; z0 < kernel_rows; ++z0) {
                    #pragma HLS LOOP_TRIPCOUNT min=1 max=3
                    for (size_t z1 = 0; z1 < kernel_cols; ++z1) {
                        #pragma HLS LOOP_TRIPCOUNT min=1 max=3
                        for (size_t q = 0; q < in_channels; ++q) {
                            #pragma HLS LOOP_TRIPCOUNT min=1 max=1
                            sum += kernel->array[z0 * (kernel->shape[3] * kernel->shape[2] * kernel->shape[1])
                                                 + z1 * (kernel->shape[3] * kernel->shape[2])
                                                 + q * (kernel->shape[3]) + k] *
                ...
                output->array[x0 * (output->shape[2] * output->shape[1]) + x1 * (output->shape[2]) + k] = sum;
            }
        }
    }
...
void k2c_pad2d(k2c_tensor* output, const k2c_tensor* input, const float fill, const size_t * pad) {
    ...
    for (size_t i1 = 0; i1 < output->numel; ++i1) {
        #pragma HLS LOOP_TRIPCOUNT min=1 max=10404
        output->array[i1] = (fabs(fill) < 1e-6) ? 0 : fill;
    }
